<x-baseLMSDashboard>
    <div id="badge" class="content-section">
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-gray-800 flex items-center">
                    <i class="fas fa-medal text-blue-600 mr-3"></i> Manajemen Badge Pencapaian
                </h3>
            ...
            <div class="form-gradient rounded-lg p-6 mb-6 hidden" id="form-container">
                <h4 class="text-lg font-semibold text-gray-700 mb-4" id="form-title">Form Badge Pencapaian</h4>
                <form id="formBadge" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="nama_pencapaian" id="nama_pencapaian"
                                    class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    placeholder="Masukkan nama pencapaian">
                            </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Kategori Pencapaian</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <i class="fas fa-tags text-gray-400"></i>
                                </div>
                                <select name="kategori_pencapaian_id" id="kategori_pencapaian_id"
                                    class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                    <option value="">Pilih Kategori</option>
                                    @foreach ($kategori as $k)
                                        <option value="{{ $k->id }}">{{ $k->nama_pencapaian }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('kategori_pencapaian_id')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Syarat (JSON)</label>
                            <div class="relative">
                                <div class="absolute top-3 left-3 pointer-events-none">
                                    <i class="fas fa-code text-gray-400"></i>
                                </div>
                                <textarea name="syarat" id="syarat" rows="4"
                                    class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    placeholder='{"min_nilai": 80, "jumlah_tugas": 5}'></textarea>
                            </div>
                            @error('syarat')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="md:col-span-2 mt-4">
                        <button type="submit"
                            ...
                            <i class="fas fa-save mr-2"></i> <span id="btn-submit-text">Simpan Badge</span>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full bg-white rounded-lg overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($badges as $b)
                            <tr>
                                <td class="px-6 py-4 text-sm">{{ $b->id }}</td>
                                <td class="px-6 py-4 text-sm">{{ $b->nama_pencapaian }}</td>
                                <td class="px-6 py-4 text-sm">{{ $b->kategori->nama_pencapaian ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm">
                                    @if ($b->gambar)
                                        <img src="{{ asset('img/badges/' . $b->gambar) }}"
                                ...
                                <td class="px-6 py-4 text-sm">{{ $b->created_at->format('Y-m-d') }}</td>
                                <td class="px-6 py-4 text-sm font-medium">
                                    <button class="text-blue-600 hover:text-blue-900 mr-3 btn-edit"
                                        data-id="{{ $b->id }}" data-nama="{{ $b->nama_pencapaian }}"
                                        data-kategori="{{ $b->kategori_pencapaian_id }}"
                                        data-syarat='@json($b->syarat)' data-gambar="{{ $b->gambar }}">
            ...
            <div class="mt-6">{{ $badges->links('pagination::tailwind') }}</div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('btn-tambah').onclick = () => {
            ...
            document.getElementById('formBadge').action = "{{ route('badge.store') }}";
            document.getElementById('btn-submit-text').textContent = 'Simpan Badge';
            preview.classList.add('hidden');
        };
        document.querySelectorAll('.btn-edit').forEach(btn => btn.onclick = () => {
            document.getElementById('form-container').classList.remove('hidden');
            document.getElementById('formBadge').reset();
            document.getElementById('badge_id').value = btn.dataset.id;
            document.getElementById('nama_pencapaian').value = btn.dataset.nama;
            document.getElementById('kategori_pencapaian_id').value = btn.dataset.kategori;
            document.getElementById('syarat').value = btn.dataset.syarat;
            document.getElementById('form_method').value = 'PUT';
            document.getElementById('formBadge').action = '/lms/dashboard/badge/' + btn.dataset.id;
            document.getElementById('btn-submit-text').textContent = 'Update Badge';
            if (btn.dataset.gambar) {
                img.src = '/img/badges/' + btn.dataset.gambar;
        ...
        document.getElementById('confirmDelete').onclick = () => {
            fetch('/lms/dashboard/badge/' + delId, {
                method: 'DELETE',
                headers: {
