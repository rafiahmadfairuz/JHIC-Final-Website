<aside id="sidebar"
    ...
    <div class="overflow-y-auto h-[calc(100%-60px)] p-4 space-y-2">
        <a href="{{ route('dashboard.penjual.produk') }}"
            class="flex items-center px-3 py-2 rounded-md
            {{ request()->routeIs('dashboard.penjual.produk') ? 'bg-gray-700 text-white' : 'hover:bg-gray-700' }}">
            <i class="fas fa-building w-5"></i>
            <span class="ml-3 sidebar-text">Produk</span>
        </a>
        <a href="{{ route('dashboard.penjual.orders') }}"
            class="flex items-center px-3 py-2 rounded-md
            {{ request()->routeIs('dashboard.penjual.orders') ? 'bg-gray-700 text-white' : 'hover:bg-gray-700' }}">
